...
/**
 * Drag-to-reveal slider. We mutate the DOM with rAF instead of going through
 * Motion values + 2 useTransform subscriptions — saves a re-render per frame.
 */
      ...
      <div className="mx-auto w-[min(1400px,92vw)]">
        <div className="grid gap-10 lg:grid-cols-[1.1fr_1fr] lg:items-end">
          ...
          >
            ...
            quiet dormancy to a softer, warmer light — captured on film,
            untouched.
          </motion.p>
        </div>
        ...
        >
          <div
            ...
            onMouseEnter={() => { setVariant('drag'); setLabel('Drag'); }}
...
function BeforeScene() {
  return (
    <div
      ...
      style={{
        background:
          'linear-gradient(135deg, #161a18 0%, #1f2421 55%, #262b27 100%)',
      }}
    >
      <div
      ...
      <div className="absolute right-[15%] bottom-[16%] h-[40%] w-[10%]">
        ...
        <div
          ...
          style={{ background: 'linear-gradient(180deg, #423d31, #20201b)' }}
        />
      </div>
  ...
  return (
    <div className="absolute inset-0">
      <div
        className="absolute inset-0"
        style={{
          background: 'linear-gradient(135deg, #f4ecdb 0%, #ece1c8 55%, #ddd0b4 100%)',
        }}
      />
      <div
        ...
        <div className="absolute inset-3 grid grid-cols-2 gap-2">
          {[0, 1, 2, 3].map((i) => (
            <div
              key={i}
              className="rounded-[2px]"
              style={{ background: 'linear-gradient(180deg, #ffffff, #d8e9de)' }}
            />
          ))}
        </div>
        <div
          className="absolute inset-0"
          style={{ background: 'linear-gradient(135deg, rgba(255,255,255,0.55) 0%, rgba(255,255,255,0) 50%)' }}
        />
      </div>
      <div
        className="absolute bottom-[12%] left-[5%] h-[26%] w-[58%] rounded-2xl"
      ...
      <div
        className="absolute inset-0"
        style={{ background: 'radial-gradient(ellipse 80% 60% at 70% 30%, rgba(255,250,235,0.35), transparent 60%)' }}
      />
    </div>
  );
}
